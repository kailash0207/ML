...
def cost_fn(X,Y,W,b):
    ...
    A[A==1]=0.999999
    A[A==0]=0.000001
    cost=-1/m*(np.dot(np.log(A),Y)+np.dot(np.log(1-A),1-Y))
    return cost
...
def write_to_csv(weights, file="WEIGHTS_FILE.csv"):
    weights=weights.reshape(4,len(weights[0][0]))
...
if __name__=="__main__":
    ...
    for i in range(4):
        label=i
        X,Y=get_data(train_X,train_Y,label)
        final_W=np.zeros((1,len(X[0])),dtype=np.float64)
        final_b=0
        A=False
        learning_rate=0.75
        while(A!=True):
